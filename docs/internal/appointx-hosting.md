# AppointX — AWS Infrastructure & Technical Specification
...
This document is the **single source of truth** for all AWS infrastructure decisions, service configurations, and technical specifications for the AppointX platform. It serves three purposes:
...
AppointX is a white-label SaaS appointment booking platform for service-based businesses in Bulgaria. Each registered business gets a branded booking page and a unique code for the mobile app.
...
| ADR-18 | Environments | dev + prod in single AWS account | Name-prefixed resources (`appointx-dev-*`, `appointx-prod-*`). Separate DynamoDB tables per environment. Separate CloudFront distributions. Single account simplifies management and avoids cross-account complexity at this stage. |
...
| Setting | Value |
|---|---|
| Function name | `appointx-{env}-api` |
| Runtime | `nodejs22.x` |
| Architecture | `arm64` |
...
| Variable | Value |
|---|---|
| `TABLE_NAME` | `appointx-{env}-main` |
| `USER_POOL_ID` | Cognito User Pool ID |
| `MEDIA_BUCKET` | `appointx-{env}-media` |
...
| `SCHEDULER_GROUP_NAME` | `appointx-{env}-reminders` |
| `ENV` | `dev` or `prod` |
...
| Setting | Value |
|---|---|
| Function name | `appointx-{env}-reminder` |
| Runtime | `nodejs22.x` |
| Architecture | `arm64` |
| Handler | `index.handler` |
| Memory | 256 MB |
...
| Setting | Value |
|---|---|
| Bucket name | `appointx-{env}-frontend` |
...
| Setting | Value |
|---|---|
| Bucket name | `appointx-{env}-media` |
| Public access | Block all |
...
3. **Configuration set:** Create `appointx-main` configuration set for delivery tracking
...
| Configuration set | `appointx-main` |
...
| Setting | Value |
|---|---|
| Android platform app | `appointx-{env}-android` (GCM/FCM) |
| iOS platform app | `appointx-{env}-ios` (APNs) |
...
| Setting | Value |
|---|---|
| Schedule group | `appointx-{env}-reminders` |
| Schedule type | One-time (`at()` expression) |
| Target | Reminder Lambda function |
| Execution role | `appointx-{env}-scheduler-role` |
...
2. For each active reminder rule on the business:
   ...
   c. Create EventBridge schedule:
      ...
      GroupName: appointx-{env}-reminders
...
#### API Lambda Role (`appointx-{env}-api-role`)

| Permission | Resource | Actions |
|---|---|---|
| DynamoDB | `appointx-{env}-main` table + all indexes | `GetItem`, `PutItem`, `UpdateItem`, `DeleteItem`, `Query`, `BatchGetItem`, `BatchWriteItem` |
| SES | Verified domain identity | `SendEmail`, `SendTemplatedEmail` |
| SNS | Platform application ARNs (`appointx-{env}-android`, `appointx-{env}-ios`) | `Publish`, `CreatePlatformEndpoint`, `DeleteEndpoint`, `GetEndpointAttributes` |
| EventBridge Scheduler | Schedule group `appointx-{env}-reminders` | `CreateSchedule`, `DeleteSchedule`, `GetSchedule` |
| S3 | `appointx-{env}-media` bucket | `PutObject` (for pre-signed URL generation) |
| CloudWatch Logs | Log group `/aws/lambda/appointx-{env}-api` | `CreateLogGroup`, `CreateLogStream`, `PutLogEvents` |
| IAM | Scheduler execution role | `iam:PassRole` (to pass the scheduler role when creating schedules) |

#### Reminder Lambda Role (`appointx-{env}-reminder-role`)

| Permission | Resource | Actions |
|---|---|---|
| DynamoDB | `appointx-{env}-main` table + all indexes | `GetItem`, `Query` (read-only) |
| SES | Verified domain identity | `SendEmail`, `SendTemplatedEmail` |
| SNS | Platform application ARNs | `Publish` |
| CloudWatch Logs | Log group `/aws/lambda/appointx-{env}-reminder` | `CreateLogGroup`, `CreateLogStream`, `PutLogEvents` |

#### EventBridge Scheduler Role (`appointx-{env}-scheduler-role`)

| Permission | Resource | Actions |
|---|---|---|
| Lambda | `appointx-{env}-reminder` function ARN | `InvokeFunction` |

#### CloudFront OAC

| Permission | Resource | Actions |
|---|---|---|
| S3 | `appointx-{env}-frontend` bucket | `GetObject` |
| S3 | `appointx-{env}-media` bucket | `GetObject` |
...
|---|---|---|
| `/appointx/{env}/ses-from-email` | String | `noreply@{domain}` |
| `/appointx/{env}/cognito-user-pool-id` | String | Cognito pool ID |
| `/appointx/{env}/cognito-client-id` | String | Cognito client ID |
| `/appointx/{env}/sns-android-arn` | String | SNS platform app ARN |
| `/appointx/{env}/sns-ios-arn` | String | SNS platform app ARN |
...
|---|---|---|
| `/aws/lambda/appointx-{env}-api` | 30 days | API Lambda |
| `/aws/lambda/appointx-{env}-reminder` | 30 days | Reminder Lambda |
| `/aws/apigateway/appointx-{env}-api` | 14 days | API Gateway access logs (optional, enable if debugging) |
...
**Alarm notification:** Create an SNS topic `appointx-ops-alerts` with email subscription for the operations team.
...
|---|---|---|---|---|
| `appointx-monthly` | COST, MONTHLY | $50 USD | At $25 (50%) | At $50 (100%) |
...
All resources follow the pattern: `appointx-{env}-{resource}`

| Environment | Prefix | Purpose |
|---|---|---|
| `dev` | `appointx-dev-*` | Development and feature testing |
| `prod` | `appointx-prod-*` | Production |

### 14.2 Resource Isolation

| Resource | Isolation Strategy |
|---|---|
| DynamoDB | Separate tables: `appointx-dev-main`, `appointx-prod-main` |
| Lambda | Separate functions: `appointx-dev-api`, `appointx-prod-api` |
| API Gateway | Separate HTTP APIs: `appointx-dev-api`, `appointx-prod-api` |
| S3 | Separate buckets per type per env |
| CloudFront | Separate distributions: dev (no custom domain, no WAF), prod (custom domain, WAF) |
| Cognito | Separate user pools: `appointx-dev-users`, `appointx-prod-users` |
| EventBridge | Separate schedule groups: `appointx-dev-reminders`, `appointx-prod-reminders` |
...
| Parameter Store | Separate paths: `/appointx/dev/*`, `/appointx/prod/*` |
...
```typescript
UserPool: {
  userPoolName: `appointx-${env}-users`,
...
ReminderFunction: {
  functionName: `appointx-${env}-reminder`,
  runtime: NODEJS_22_X,
  architecture: ARM_64,
  handler: 'index.handler',
  memorySize: 256,
...
```typescript
ScheduleGroup: {
  name: `appointx-${env}-reminders`,
}

SchedulerRole: {
  roleName: `appointx-${env}-scheduler-role`,
...
```typescript
FrontendBucket: {
  bucketName: `appointx-${env}-frontend`,
...
Create an IAM role `appointx-github-deploy` with trust policy for the GitHub OIDC provider, scoped to the specific repository.
...
Steps:
  ...
  6. Package and deploy:
     aws lambda update-function-code --function-name appointx-prod-api --zip-file ...
     aws lambda update-function-code --function-name appointx-prod-reminder --zip-file ...
...
Steps:
  ...
  5. aws s3 sync dist/ s3://appointx-prod-frontend --delete
